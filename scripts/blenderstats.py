...
def run_stats(results, probs, name, plot, repeat_prob):
    ...
    print("DEVIATION: {} ({})".format(
        __time_to_str(statistics.stdev(times_p)),
        statistics.stdev(times_p)))
    print("VARIANCE: {} ({})".format(
        __time_to_str(statistics.variance(times_p)),
        statistics.variance(times_p)))
    ...
    if repeat_prob:
        ...
        min_ = min(estm_times)
        max_ = max(estm_times)
        print("100 PROBS ESTM_TIME: {} ({})"
              .format(__time_to_str(n*mean), n*mean))
        print("100 PROBS MEAN: {} ({})".format(__time_to_str(mean), mean))
        print("100 PROBS DEVIATION: {} ({})"
              .format(__time_to_str(std_dev), std_dev))

...
class TestDockerLuxrenderTask(TempDirFixture, DockerTestCase):
    ...
    def _extract_results(self, computer, task, subtask_id):
        """
        Since the local computer use temp dir, you should copy files out of
        there before you use local computer again. Otherwise the files would get
        overwritten (during the verification process). This is a problem only in
         test suite. In real life provider and requestor are separate machines
        :param computer:
        :param task:
        :return:
        """
    ...
    def _test_luxrender_real_task(self, task):
        ...
        task.computation_finished(
            ctd.subtask_id, [new_flm_file, new_preview_file],
        ...
        ctd = task.query_extra_data(10000).ctd
        task.computation_finished(
            ctd.subtask_id, [bad_flm_file, new_preview_file],
            result_type=result_types["files"])
    ...
    def test_run_stats(self):
        ...
        for i in range(0, 10):
            ...
            self.assertEqual(task.num_tasks_received, 0)
            task.computation_finished(
                ctd.subtask_id, [new_flm_file, new_png_file],
                result_type=result_types["files"])

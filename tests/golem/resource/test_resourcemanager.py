...
class TestResourcesManager(TempDirFixture):
    ...
    def testGetResourceHeader(self):
        ...
        self.assertEqual(len(header2.files_data), 0)
        self.assertEqual(len(header2.sub_dir_headers), 0)

    def testGetResourceDelta(self):
        rm = ResourcesManager(self.dir_manager, 'owner')
        header = rm.get_resource_header('task2')
        delta = rm.get_resource_delta('task2', header)
        self.assertEqual(len(delta.files_data), 0)
        self.assertEqual(len(delta.sub_dir_resources[0].files_data), 0)
        header2 = rm.get_resource_header('task3')
        delta2 = rm.get_resource_delta('task2', header2)
        self.assertEqual(len(delta2.files_data), 2)
        self.assertEqual(len(delta2.sub_dir_resources[0].files_data), 1)
        res_path = self.dir_manager.get_task_resource_dir('task2')
        ...
        self.assertEqual(len(delta3.files_data), 1)
        self.assertEqual(len(delta3.sub_dir_resources[0].files_data), 1)
        os.remove(file4)
        os.remove(file5)

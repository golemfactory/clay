...
class TestIncomesKeeper(TestWithDatabase, PEP8MixIn):
    ...
    def _test_expect_income(self, payer_address, subtask_id, value):
    ...
    def test_received_batch_transfer_closure_time(self):
        ...
        income1 = Income.get(payer_address=payer_address, subtask=subtask_id1)
        assert income1.transaction is None
        income2 = Income.get(payer_address=payer_address, subtask=subtask_id2)
    ...
    def test_received_batch_transfer_two_senders(self):
        ...
        assert Income.select().count() == 0
        self._test_expect_income(
            payer_address=payer_address1,
            subtask_id=subtask_id1,
            value=value1,
        )
        self._test_expect_income(
            payer_address=payer_address2,
            subtask_id=subtask_id2,
            value=value2,
        )
        assert Income.select().count() == 2
        ...
        income1 = Income.get(payer_address=payer_address1, subtask=subtask_id1)
        assert transaction_id1[2:] == income1.transaction
        income2 = Income.get(payer_address=payer_address2, subtask=subtask_id2)
        assert income2.transaction is None

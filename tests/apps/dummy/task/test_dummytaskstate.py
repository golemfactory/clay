...
class TestDummyTaskDefaults(TestCase):
    def test_init(self):
        ...
        assert td.options.difficulty == 10
...
class TestDummyTaskDefinition(TempDirFixture):
    def test_init(self):
        ...
        assert tdd.options.difficulty == 10
    ...
    def test_add_to_resources(self):
        ...
        with patch("tempfile.mkdtemp", lambda: self.tempdir):
            ...
            assert os.path.islink(os.path.join(td.tmp_dir, "code"))
            assert os.path.islink(os.path.join(td.tmp_dir, "data"))

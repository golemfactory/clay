...
from apps.jee4g.environment import Jee4gTaskEnvironment
...
class Jee4gTaskOptions(Options):
    ...
    def __init__(self) -> None:
        ...
        self.input_dir: str = ''
        self.output_dir: str = ''
        self.subtasks: Dict[str, Jee4gTaskOptions.SubtaskOptions] = {}
...
class Jee4gTaskDefinition(TaskDefinition):
    def __init__(self) -> None:
        super().__init__()
        self.options = Jee4gTaskOptions()
        self.task_type = 'JEE4G'

    def add_to_resources(self) -> None:
        self.resources = [self.options.input_dir]


class Jee4gTaskVerifier(CoreVerifier):
    def __init__(self,
...
class Jee4gTask(CoreTask):
    ENVIRONMENT_CLASS = Jee4gTaskEnvironment
    VERIFIER_CLASS = Jee4gTaskVerifier

    JOB_ENTRYPOINT = '/golem/scripts/job.sh'

    def __init__(self, total_tasks: int, task_definition: Jee4gTaskDefinition,
        ...
        self.options: Jee4gTaskOptions = task_definition.options
...
class Jee4gTaskBuilder(CoreTaskBuilder):
    TASK_CLASS: Type[Jee4gTask] = Jee4gTask

    @classmethod
    def build_full_definition(cls, task_type: 'CoreTaskTypeInfo',
                              dictionary: Dict[str, Any]) -> Jee4gTaskDefinition:
        ...
        task_def.options.jar_name = options['jar_name']
...
class Jee4gBenchmarkTask(Jee4gTask):
    def query_extra_data(self, perf_index: float, node_id: Optional[str] = None,
                         node_name: Optional[str] = None) -> Task.ExtraData:
...
class Jee4gBenchmarkTaskBuilder(Jee4gTaskBuilder):
    TASK_CLASS: Type[Jee4gTask] = Jee4gBenchmarkTask


class Jee4gTaskTypeInfo(CoreTaskTypeInfo):
    def __init__(self) -> None:
        super().__init__(
            'JEE4G', Jee4gTaskDefinition, Jee4gTaskOptions, Jee4gTaskBuilder
        )

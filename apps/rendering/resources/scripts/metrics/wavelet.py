...
class MetricWavelet:
    ...
    #
    @staticmethod
    def compute_metrics(image1, image2):  # pylint: disable=too-many-statements
        ...
        for i in range(0, 3):
            ...
            result[
                "wavelet_db4_low"] = result["wavelet_db4_low"] + calculate_mse(
                    coeff1, coeff2, 1, 1 + len_div_3)
            result[
                "wavelet_db4_mid"] = result["wavelet_db4_mid"] + calculate_mse(
                    coeff1, coeff2, 1 + len_div_3, 1 + len_two_thirds)
            result["wavelet_db4_high"] = result[
                "wavelet_db4_high"] + calculate_mse(
                    coeff1, coeff2, 1 + len_two_thirds, 1 + len_total)
        ...
        for i in range(0, 3):
            ...
            result["wavelet_sym2_low"] = result[
                "wavelet_sym2_low"] + calculate_mse(coeff1, coeff2, 1,
                                                    1 + len_div_3)
            result["wavelet_sym2_mid"] = result[
                "wavelet_sym2_mid"] + calculate_mse(
                    coeff1, coeff2, 1 + len_div_3, 1 + len_two_thirds)
            result["wavelet_sym2_high"] = result[
                "wavelet_sym2_high"] + calculate_mse(
                    coeff1, coeff2, 1 + len_two_thirds, 1 + len_total)
        ...
        for i in range(0, 3):
            ...
            result["wavelet_haar_freq_x1"] = result[
                "wavelet_haar_freq_x1"] + freqs[0]
            result["wavelet_haar_freq_x2"] = result[
                "wavelet_haar_freq_x2"] + freqs[1]
            result["wavelet_haar_freq_x3"] = result[
                "wavelet_haar_freq_x3"] + freqs[2]
    ...
    def get_labels():
        return [
            "wavelet_sym2_base", "wavelet_sym2_low", "wavelet_sym2_mid",

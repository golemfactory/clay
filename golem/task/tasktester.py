...
class TaskTester(LocalComputer):
    ...
    def computation_success(self, task_thread):
        ...
        after_test_data = self.task.after_test(res, self.tmp_dir, time_spent)
        self.success_callback(res, est_mem, time_spent, msg=after_test_data.get("warnings"),
                              estm_time=after_test_data.get("estm_time"))

...
class TaskSession(MiddlemanSafeSession):
    ...
    def send_report_computed_task(
        ...
        self.send(
            message.MessageReportComputedTask(
                task_result.subtask_id,
                task_result.result_type,
                task_result.computing_time,
                node_name,
                address,
                port,
        ...
        if wrong_task:
            self.send(
                message.MessageCannotAssignTask(
                    msg.task_id,
                    "Not my task  {}".format(msg.task_id)
                )
            )
            self.dropped()
        elif ctd:
            ...
            self.send(
                message.MessageCannotAssignTask(
                    msg.task_id,
                    "No more subtasks in {}".format(msg.task_id)
                )
            )
            self.dropped()
    ...
    def _react_to_task_to_compute(self, msg):
        if self._check_ctd_params(msg.ctd)\
                and self._set_env_params(msg.ctd)\
                and self.task_manager.comp_task_keeper.receive_subtask(msg.ctd):  # noqa
    ...
    def _react_to_resource_format(self, msg):
        if not msg.use_distributed_resource:
            tmp_dir = self.task_computer.resource_manager.get_temporary_dir(self.task_id)  # noqa
            tmp_file = os.path.join(tmp_dir, "res" + self.task_id)
            output_dir = self.task_computer.resource_manager.get_resource_dir(self.task_id)  # noqa
            extra_data = {
    ...
    def __send_resource_parts_list(self, msg):
        ...
        self.send(message.MessageDeltaParts(self.task_id, delta_header, parts_list, self.task_server.get_node_name(), self.task_server.node, self.task_server.get_resource_addr(), self.task_server.get_resource_port()))  # noqa
    ...
    def __send_result_hash(self, res):
        ...
        def success(result):
            ...
            logger.debug(
                "Task session: sending task result hash: {} ({})",
                result_path,
                result_hash
            )
        ...
        def error(exc):
            ...
            if isinstance(exc, EnvironmentError):
                ...
                self.send(
                    message.MessageTaskFailure(subtask_id, '{}'.format(exc))
                )
    ...
    def __set_msg_interpretations(self):
        self._interpretation.update({
            ...
            message.MessageAcceptResourceFormat.TYPE: self._react_to_accept_resource_format,  # noqa

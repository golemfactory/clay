...
use libp2p::core::{ConnectedPoint, Endpoint, PeerId, ProtocolsHandler, ProtocolsHandlerEvent};
    ...
    TSubstream: AsyncRead + AsyncWrite,
{
        ...
        self.state = match mem::replace(&mut self.state, ProtocolState::Poisoned) {
            ProtocolState::Poisoned => {
                error!(
                    ...
                    "Handler with {:?} is in poisoned state", self.remote_peer_id
                );
                ProtocolState::Poisoned
            }
            ...
            ProtocolState::Init {
                ...
                match init_deadline.poll() {
                    Ok(Async::Ready(())) => {
                        ...
                        error!(
                            target: crate::LOG_TARGET,
                            "Handler initialization process is too long \
                             with {:?}",
                            self.remote_peer_id
                        )
                    }
        ...
        self.state = match mem::replace(&mut self.state, ProtocolState::Poisoned) {
            ...
            ProtocolState::Init {
                ...
                if substream.endpoint() == Endpoint::Dialer {
                    error!(
                        target: crate::LOG_TARGET,
                        "Opened dialing substream with {:?} before \
                         initialization",
                        self.remote_peer_id
                    );
                }
    ...
    fn send_message(&mut self, message: ProtocolMessage) {
        match self.state {
            ...
            _ => debug!(
                target: crate::LOG_TARGET,
                "Tried to send message over closed protocol \
                 with {:?}",
                self.remote_peer_id
            ),
        }

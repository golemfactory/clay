...
impl RegisteredProtocol {
    ...
    pub fn new(protocol: ProtocolId, mut versions: Vec<u8>) -> Self {
        let mut base_name = Bytes::from_static(b"/");
        base_name.extend_from_slice(crate::PROTOCOL_NAME.as_bytes());
...
impl<TSubstream> RegisteredProtocolSubstream<TSubstream> {
    ...
    pub fn send_message(&mut self, data: ProtocolMessage) {
        if self.is_closing {
            return;
        }
    ...
    fn poll(&mut self) -> Poll<Option<Self::Item>, Self::Error> {
            ...
            if !self.clogged_fuse {
                ...
                return Ok(Async::Ready(Some(RegisteredProtocolEvent::Clogged {
                    messages: self
                        .send_queue
                        .iter()
                        .map(|m| ProtocolMessage::Blob(self.protocol_id, m.clone()))
                        .collect(),
                })));
            }
        } else {
        ...
        match self.inner.poll()? {
            Async::Ready(Some(data)) => {
                let message = ProtocolMessage::Blob(self.protocol_id, data.to_vec());

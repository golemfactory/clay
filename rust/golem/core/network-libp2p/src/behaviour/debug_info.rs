...
use libp2p::core::protocols_handler::{
    IntoProtocolsHandler, IntoProtocolsHandlerSelect, ProtocolsHandler,
};
...
impl<TSubstream> DebugInfoBehaviour<TSubstream> {
    ...
    pub fn new(user_agent: String, local_public_key: PublicKey) -> Self {
        let identify = {
            let proto_version: String = format!("/{}/{}", PROTOCOL_NAME, PROTOCOL_VERSION);
...
impl<'a> Node<'a> {
    ...
    pub fn debug_info(&self) -> String {
        format!(
            "(version: {:?}) through {:?}",
            self.0.client_version, self.0.endpoint
        )
    }
}
            ...
            match self.identify.poll(params) {
                ...
                Async::Ready(NetworkBehaviourAction::GenerateEvent(event)) => match event {
                    ...
                    IdentifyEvent::SendBack {
                        ...
                        target: crate::LOG_TARGET,
                        "Error when sending back identify info \
                         to {:?} => {}",
                        peer_id,
                        err
                    ),

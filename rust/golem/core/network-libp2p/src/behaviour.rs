...
impl<TBehaviour, TBehaviourEv, TSubstream> Behaviour<TBehaviour, TBehaviourEv, TSubstream> {
	...
	pub fn new(
		...
		if enable_mdns {
			...
			warn!(target: crate::LOG_TARGET, "mDNS is not available on this platform");
		}
...
impl<TBehaviour, TBehaviourEv, TSubstream> NetworkBehaviourEventProcess<debug_info::DebugInfoEvent>
	...
	fn inject_event(&mut self, event: debug_info::DebugInfoEvent) {
		...
		if !info.protocol_version.contains(crate::PROTOCOL_NAME) {
			warn!(target: crate::LOG_TARGET, "Connected to a non-{:?} node: {:?}",
			      crate::PROTOCOL_NAME, info);
		}
		if info.listen_addrs.len() > 30 {
			warn!(target: crate::LOG_TARGET, "Node {:?} has reported more than 30 addresses; \
		...
		match out {
			...
			KademliaOut::FindNodeResult { key, closer_peers } => {
				...
				if closer_peers.is_empty() {
					debug!(target: crate::LOG_TARGET, "Libp2p => Random Kademlia query has yielded empty \
						   results");
				}
			}
	...
	TSubstream: AsyncRead + AsyncWrite,
{
	...
	fn inject_new_external_addr(&mut self, addr: &Multiaddr) {
		...
		info!(target: crate::LOG_TARGET, "Discovered new external address for our node: {}", new_addr);
	}

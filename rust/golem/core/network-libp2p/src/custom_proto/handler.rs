	...
	TMessage: CustomMessage,
{
	...
	fn enable(&mut self, endpoint: Endpoint) {
		self.state = match mem::replace(&mut self.state, ProtocolState::Poisoned) {
			ProtocolState::Poisoned => {
				error!(target: crate::LOG_TARGET, "Handler with {:?} is in poisoned state",
					self.remote_peer_id);
				ProtocolState::Poisoned
			}
		...
		self.state = match mem::replace(&mut self.state, ProtocolState::Poisoned) {
			...
			ProtocolState::Init { substreams, mut init_deadline } => {
				match init_deadline.poll() {
					Ok(Async::Ready(())) => {
						...
						error!(target: crate::LOG_TARGET, "Handler initialization process is too long \
							with {:?}", self.remote_peer_id)
					},
					Ok(Async::NotReady) => {}
					Err(_) => error!(target: crate::LOG_TARGET, "Tokio timer has errored")
				}
			...
			ProtocolState::Opening { mut deadline } => {
				match deadline.poll() {
					...
					Err(_) => {
						error!(target: crate::LOG_TARGET, "Tokio timer has errored");
						deadline.reset(Instant::now() + Duration::from_secs(60));
						return_value = None;
						ProtocolState::Opening { deadline }
					},
		...
		self.state = match mem::replace(&mut self.state, ProtocolState::Poisoned) {
			...
			ProtocolState::Init { mut substreams, init_deadline } => {
				if substream.endpoint() == Endpoint::Dialer {
					error!(target: crate::LOG_TARGET, "Opened dialing substream with {:?} before \
			...
			ProtocolState::BackCompat { substream: existing, mut shutdown } => {
				warn!(target: crate::LOG_TARGET, "Received extra substream after having already one \
			...
			ProtocolState::Normal(mut state) => {
				if substream.endpoint() == Endpoint::Listener {
					...
					debug!(target: crate::LOG_TARGET, "Opened spurious outbound substream with {:?}",
						self.remote_peer_id);
					substream.shutdown();
					state.shutdown.push(substream);
				}
	...
	fn send_message(&mut self, message: TMessage) {
		match self.state {
			...
			ProtocolState::Normal(ref mut state) => {
				if let CustomMessageId::Request(request_id) = message.request_id() {
					...
					if let Some(pos) = state.pending_send_back.iter().position(|(id, _)| *id == request_id) {
						...
						warn!(target: crate::LOG_TARGET, "Libp2p layer received response to a \
			...
			_ => debug!(target: crate::LOG_TARGET, "Tried to send message over closed protocol \
				with {:?}", self.remote_peer_id)
		}
	}
}

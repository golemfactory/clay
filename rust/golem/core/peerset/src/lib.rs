...
impl PeersetHandle {
	/// Dials a multiaddr.
	pub fn dial_addr(&self, multiaddr: Multiaddr) {
		let _ = self.tx.unbounded_send(Action::Dial(multiaddr));
	}
...
pub enum Message {
	/// Request to open a connection to the given address.
	Dial(Multiaddr),
...
impl Peerset {
	...
	fn on_dial_addr(&mut self, multiaddr: Multiaddr) {
		self.message_queue.push_back(Message::Dial(multiaddr));
	}
		...
		loop {
			...
			match try_ready!(self.rx.poll()) {
				...
				Some(action) => match action {
					Action::Dial(multiaddr) => self.on_dial_addr(multiaddr),

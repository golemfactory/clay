...
#[derive(Debug)]
pub enum Message {
	/// Request to open a connection to a given address.
	Dial(Multiaddr),
...
impl Peerset {
	...
	pub fn dial_addr(&self, multiaddr: Multiaddr) {
		let _ = self.tx.unbounded_send(Message::Dial(multiaddr));
	}
	...
	pub fn remove_reserved_peer(&self, peer_id: &PeerId) -> bool {
		let mut inner = self.inner.lock();

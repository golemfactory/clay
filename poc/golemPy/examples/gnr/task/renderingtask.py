...
class RenderingTaskBuilder(GNRTaskBuilder):
    ...
    def _set_verification_options(self, new_task):
        if self.task_definition.verification_options is None:
            ...
            new_task.verification_options.box_size = (self.task_definition.verification_options.box_size[0], (self.task_definition.verification_options.box_size[1] / 2) * 2)
...
class RenderingTask(GNRTask):
    ...
    def _put_collected_files_together(self, output_file_name, files, arg):
        if is_windows():
            task_collector_path = os.path.normpath(os.path.join(os.environ.get('GOLEM'), "tools/taskcollector/Release/taskcollector.exe"))
        else:
            task_collector_path = os.path.normpath(os.path.join(os.environ.get('GOLEM'), "tools/taskcollector/Release/taskcollector"))
    ...
    def _verify_imgs(self, subtask_id, tr_files):
        ...
        for tr_file in tr_files:
            if adv_test_file is not None and tr_file in adv_test_file:
                ...
                if not advance_verify_img(tr_file, res_x, res_y, start_box, self.verification_options.box_size, cmp_file, cmp_start_box):
                    return False
                else:

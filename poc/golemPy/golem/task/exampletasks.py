...
class VRayTracingTask(Task):
    ...
    def short_extra_data_repr(self, perf_index):
        if self.last_extra_data:
            ...
            return "x: {}, y: {}, w: {}, h: {}, num_pixels: {}, num_samples: {}".format(l["x"], l["y"], l["w"], l["h"],
                                                                                        l["num_pixels"],
                                                                                        l["num_samples"])
...
class PbrtRenderTask(Task):
    def __init__(self, header, path_root, total_tasks, num_subtasks, num_cores, outfilebasename, scene_file,
                 return_address="", return_port=0):
    ...
    def __short_extra_data_repr(self, perf_index, extra_data):
        l = extra_data
        return "path_root: {}, start_task: {}, end_task: {}, total_tasks: {}, num_subtasks: {}, num_cores: {}, outfilebasename: {}, scene_file: {}".format(
            l["path_root"], l["start_task"], l["end_task"], l["total_tasks"], l["num_subtasks"], l["num_cores"],
            l["outfilebasename"], l["scene_file"])
    ...
    def computation_finished(self, subtask_id, task_result, env=None):
        ...
        if len(task_result) > 0:
            for trp in task_result:
                ...
                self.collector.add_img_file(
                    os.path.join(tmp_dir, tr[0]))  # pewnie tutaj trzeba czytac nie zpliku tylko z streama
                self.num_tasks_received += 1

        if self.num_tasks_received == self.total_tasks:
            self.collector.finalize().save(
                "{}.png".format(os.path.join(env.get_task_output_dir(self.header.task_id), "test")), "PNG")

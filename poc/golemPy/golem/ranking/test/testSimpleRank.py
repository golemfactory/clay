...
class TestSimpleRank(unittest.TestCase):
    ...
    def testNetworkRank(self) :
        ...
        # 4 chce liczyc
        node1.set_node_rank('node4', {'node2': node2.get_node_rank('node4'), 'node3': node3.get_node_rank('node4') })
        node2.set_node_rank('node4', {'node1': node1.get_node_rank('node4'), 'node3': node3.get_node_rank('node4') })
        node3.set_node_rank('node4', {'node2': node2.get_node_rank('node4'), 'node1': node1.get_node_rank('node4') })
        node4.set_node_rank('node1', {'node2': node2.get_node_rank('node1') })
        node4.set_node_rank('node2', {'node1': node1.get_node_rank('node2') })
        node4.set_node_rank('node3', {'node2': node2.get_node_rank('node3'), 'node1': node1.get_node_rank('node3') })
        print "Node1: {}".format(node1.ranking)
        print "Node2: {}".format(node2.ranking)
        print "Node3: {}".format(node3.ranking)

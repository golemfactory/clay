...
class TestSimpleRank( unittest.TestCase ):
    ...
    def testNetworkRank( self ) :
        ...
        node2.setNodeRank( 'node3', { 'node1': node1.getNodeRank('node3' ) } )
        node3.setNodeRank( 'node2', { 'node2': node1.getNodeRank( 'node2' ) } )
        print "Node1: {}".format( node1.ranking )
        print "Node2: {}".format( node2.ranking )
        print "Node3: {}".format( node3.ranking )
        # 2 chce cos policzyc
        node1.setNodeRank( 'node2', { 'node3': node1.getNodeRank('node2' ) } )
        node3.setNodeRank( 'node2', { 'node1': node3.getNodeRank( 'node2' ) } )
        print "Node1: {}".format( node1.ranking )
        print "Node2: {}".format( node2.ranking )
        print "Node3: {}".format( node3.ranking )
        ...
        # 4 chce liczyc
        node1.setNodeRank( 'node4', {'node2': node2.getNodeRank('node4'), 'node3': node3.getNodeRank('node4' ) } )
        node2.setNodeRank( 'node4', {'node1': node1.getNodeRank('node4'), 'node3': node3.getNodeRank('node4' ) } )
        node3.setNodeRank( 'node4', {'node2': node2.getNodeRank('node4'), 'node1': node1.getNodeRank('node4' ) } )
        node4.setNodeRank( 'node1', {'node2': node2.getNodeRank('node1') } )
        node4.setNodeRank( 'node2', {'node1': node1.getNodeRank('node2') } )
        node4.setNodeRank( 'node3', {'node2': node2.getNodeRank('node3'), 'node1': node1.getNodeRank('node3' ) } )
        print "Node1: {}".format( node1.ranking )
        print "Node2: {}".format( node2.ranking )
        print "Node3: {}".format( node3.ranking )
